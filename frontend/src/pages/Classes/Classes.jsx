      ...
      <div className='my-16 w-[90%]  mx-auto grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
        {
          classes.map((cls, index) => (
            <div
            key={index}
            className={`relative flex flex-col hover:-translate-y-2 duration-150 hover:ring-[2px] hover:ring-secondary w-64 min-h-[350px] mx-auto ${cls.availableSeats < 1 ? 'bg-red-300' : 'bg-white'} dark:bg-dark rounded-lg shadow-lg overflow-auto cursor-pointer`}
            ...
            >
              ...
              <h3 
                className='font-bold text-lg text-dark truncate max-w-full' 
                title={cls.name}
                >
                  {cls.name}
                </h3>
